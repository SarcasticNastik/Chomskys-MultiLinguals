...
def sort_all(batch, lens):
    ...
    sorted_all = [list(t) for t in
                  zip(*sorted(zip(*unsorted_all), reverse=True))]
    return sorted_all[2:], sorted_all[1]


def get_augment_ratio(train_data, should_augment_predicate,
                      can_augment_predicate, desired_ratio=0.1, max_ratio=0.5):
    """
    ...
    n_can_augment = sum(
        can_augment_predicate(sentence) for sentence in train_data)
    n_error = sum(
        can_augment_predicate(sentence) and not should_augment_predicate(
            sentence)
...
def can_augment_nopunct_predicate(sentence):
    ...
    if len(last_word[ID]) > 1:
        return False
    if any(len(word[ID]) == 1 and word[HEAD] == last_word[ID][0] for word in
           sentence):
        return False
    return True
...
def augment_punct(train_data, augment_ratio,
    ...
    if augment_ratio is None:
        augment_ratio = get_augment_ratio(train_data, should_augment_predicate,
                                          can_augment_predicate)
