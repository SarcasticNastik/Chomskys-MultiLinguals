...
from utils.trainer import Trainer as BaseTrainer
from utils import utils, loss
from utils.chuliu_edmonds import chuliu_edmonds_one_root
from depparse.model import Parser
from utils.pos_vocab import MultiVocab
...
class Trainer(BaseTrainer):
    ...
    def __init__(self, args=None, vocab=None, pretrain=None, model_file=None,
        ...
        if model_file is not None:
            ...
            self.args = args
            self.vocab = vocab
            self.model = Parser(args, vocab,
                                emb_matrix=pretrain.emb if pretrain is not None else None)
        self.parameters = [p for p in self.model.parameters() if
                           p.requires_grad]
        if self.use_cuda:
            self.model.cuda()
        else:
    ...
    def predict(self, batch, unsort=True):
        ...
        head_seqs = [chuliu_edmonds_one_root(adj[:l, :l])[1:] for adj, l in
                     zip(preds[0], sentlens)]  # remove attachment for the root
        deprel_seqs = [self.vocab['deprel'].unmap(
            [preds[1][i][j + 1][h] for j, h in enumerate(hs)]) for i, hs in
                       enumerate(head_seqs)]

        pred_tokens = [[[str(head_seqs[i][j]), deprel_seqs[i][j]] for j in
                        range(sentlens[i] - 1)] for i in range(batch_size)]
        if unsort:
    ...
    def save(self, filename, skip_modules=True):
        ...
        if skip_modules:
            skipped = [k for k in model_state.keys() if
                       k.split('.')[0] in self.model.unsaved_modules]
            for k in skipped:
                del model_state[k]
        params = {
            'model': model_state,
